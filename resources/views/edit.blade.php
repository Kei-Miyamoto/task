    ...
    <div class="card form-card">
      ...
      <form method="POST" action="{{ route('update') }}"  class="card-body" onSubmit="return checkSubmit()" enctype="multipart/form-data">
        ...
        <div class="form-group row col- form-row">
          ...
          <input name="product_id" class="form-control col-xs-12 col-sm-8 col-md-8"class="form-control col-xs-12 col-sm-8 col-md-8" type="text" value="{{ $product_edit->id }}" readonly>
        </div>
  
        <div class="form-group row col- form-row">
          <label class="col-xs-12 col-sm-4 col-md-4 col-form-label" for="product_name">商品名</label>
          <input name="product_name" class="form-control col-xs-12 col-sm-8 col-md-8" type="text" value="{{ $product_edit->product_name }}">
        ...
        <div class="form-group row col- form-row">
          ...
          <input type="text" class="form-control col-xs-12 col-sm-8 col-md-8" name="price" value="{{ $product_edit->price }}">
          @if ($errors->has('price'))
            <div class="text-danger">
              {{ $errors->first('price') }}
            </div>
        ...
        <div class="form-group row col- form-row">
          ...
          <input type="text" class="form-control col-xs-12 col-sm-8 col-md-8" name="stock" value="{{ $product_edit->stock }}">
          @if ($errors->has('stock'))
            <div class="text-danger">
              {{ $errors->first('stock') }}
            </div>
        ...
        <div class="form-group row col- form-row">
          ...
          <textarea type="text" class="form-control col-xs-12 col-sm-8 col-md-8" name="comment" >{{ $product_edit->comment }}</textarea>
        ...
        <div class="mt-5">
          ...
          <button type="submit" class="btn btn-primary edit-btn">更新</button>
        </div>
      </form>
    </div>
  </div>
